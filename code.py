...
def create_positional_embedding(max_len, embed_dim):
    ...
    pe[:, 0, 1::2] = np.cos(positions * div_term)
...
class TransformerEncoder(nn.Module):
    def __init__(self, src_vocab, embedding_dim, num_heads,
        ...
        self.encoder = nn.TransformerEncoder(
            nn.TransformerEncoderLayer(embedding_dim, num_heads, dim_feedforward, dropout),
            num_layers
            ).to(device)
    ...
    def forward(self, x):
        ...
        # Pass x through the word embedding
        x_embedded = self.embedding(x.to(self.device))
        
        # Add positional embedding to the word embedding, then apply dropout
        x_embedded = self.dropout(x_embedded + self.positional_embedding[:x.size(0), :])
        
        # Call make_src_mask(x) to compute a mask
        src_mask = self.make_src_mask(x)
        
        # Call the encoder, with src_key_padding_mask = src_mask
        output = self.encoder(x_embedded, src_key_padding_mask = src_mask)
...
class TransformerDecoder(nn.Module):
    def __init__(self, trg_vocab, embedding_dim, num_heads,
        ...
        self.decoder = nn.TransformerDecoder(
            nn.TransformerDecoderLayer(embedding_dim, num_heads, dim_feedforward, dropout),
            num_layers
        ).to(device)

        # Final fully connected layer
        self.fc_out = nn.Linear(embedding_dim, trg_vocab_size).to(device)
    ...
    def forward(self, dec_in, enc_out):
        ...
        dec_embedded = self.embedding(dec_in.to(self.device))
        dec_embedded = self.dropout(dec_embedded + self.positional_embedding[:dec_in.size(0), :])

        # Call generate_square_subsequent_mask() to compute a mask
        trg_mask = self.generate_square_subsequent_mask(dec_in.size(0))

        # Call the decoder, with tgt_mask = trg_mask
        output = self.decoder(dec_embedded, enc_out.to(self.device), tgt_mask = trg_mask)
...
def decode_transformer_model(encoder, decoder, src, max_decode_len, device):
    ...
    # For 1 ≤ t ≤ max_decode_len
    for t in range(1, max_decode_len):
        # Obtain dec_input as the best words so far for previous time steps
        dec_input = curr_output[:, :t].long().transpose(0, 1)

        # Obtain your (unnormalized) prediction probabilities by feeding dec_input and encoder output to decoder
        curr_pred_probs = decoder(dec_input, enc_output)

        # Save your (unnormalized) prediction probabilities in curr_predictions at index t
        curr_predictions[:, t, :] = curr_pred_probs[-1, :, :]

        # Calculate the most likely (highest probability) token and save in curr_output at timestep t
        curr_output[:, t] = curr_pred_probs.argmax(dim = -1)[-1, :]
